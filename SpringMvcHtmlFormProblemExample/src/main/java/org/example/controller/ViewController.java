...
public class ViewController {
    ...
    public String getProfileView(@PathVariable("userId") long userId, Model model) {
        ...
        model.addAttribute("status", user.status());
        model.addAttribute(
                "age",
    ...
    @GetMapping("/profile/set_status/{id}")
    public String getStatusForm(@PathVariable("id") long id, Model model){
        User user = userService.getUser(id);
        UserFormData userFormData = new UserFormData();
        userFormData.setStatus(user.status());
        model.addAttribute("setStatusForm", userFormData);
        model.addAttribute("id", id);
        return "set_user_status_form";
    }

    @PatchMapping("/profile/set_status/{id}")
    public String setUserStatus(@PathVariable("id") long id,
                                @ModelAttribute("setStatusForm") UserFormData userFormData,
                                BindingResult result, Model model){
        if (result.hasErrors()) {
            return "user_profile_edit_error";
        }
        User updatedUserDto = userService.updateUser(id, userFormData.toUser());
        model.addAttribute("updatedUser", updatedUserDto);
        return "user_profile_edit_success";
    }
